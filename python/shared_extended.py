...
class OPAYGOSharedExtended(object):
    ...
    def generate_next_token(cls, last_code, key):
        # We convert the token to bytes
        conformed_token = struct.pack('>Q', last_code)
        token_hash = siphash.SipHash_2_4(
            key, conformed_token).hash()  # We hash it
    ...
    def _convert_hash_to_token(cls, this_hash):
        ...
        token = cls._convert_to_40_bits(this_hash)
        return token
